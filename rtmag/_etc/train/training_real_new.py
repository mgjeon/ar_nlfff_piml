...
def shared_step(model, sample_batched, args):
    # input --------------------------------------------------------------
    # [b, 3, x, y, 1] -> [b, 3, x, y]
    inputs = sample_batched['input'].to(device)[:, :, :, :, 0]
    # [b, 3, x, y] -> [b, 3, y, x]
    ...
    # [b, 3*256, y, x] -> [b, 256, y, x, 3]
    outputs = torch.stack([outputs[:, 0:256], outputs[:, 256:512], outputs[:, 512:768]], dim=-1)
...
def val_plot(model, test_dataloader, epoch, args, writer):
    with torch.no_grad():
        ...
        # [b, 3, x, y, 1] -> [b, 3, x, y]
        inputs = batch['input'].to(device)[:, :, :, :, 0]
        # [b, 3, x, y] -> [b, 3, y, x]
        inputs = torch.permute(inputs, (0, 1, 3, 2))
        # [b, 3, y, x] -> [b, 3*256, y, x]
