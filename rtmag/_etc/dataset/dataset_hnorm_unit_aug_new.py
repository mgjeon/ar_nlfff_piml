...
class ISEEDataset_Hnorm_Unit_Aug_New(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = inputs[:, :-1, :-1, :]
        inputs = torch.clip(inputs, -6000, 6000)
        ...
        labels = labels[:, :-1, :-1, :-1]
        labels = torch.clip(labels, -6000, 6000)

        aa = np.arange(1, labels.shape[-1]//8, 0.5)
        bb = np.arange(labels.shape[-1]//8, labels.shape[-1]//4 + 1, 2)
        cc = np.arange(labels.shape[-1]//4 + 1, 2*labels.shape[-1] + 260, 4)
        divi = np.concatenate([aa, bb, cc])
        divisor = (self.b_norm / divi).reshape(1, 1, 1, -1).astype(np.float32)
...
class ISEEDataset_Multiple_Hnorm_Unit_Aug_New(Dataset):
