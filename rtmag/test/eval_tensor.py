...
from rtmag.test.diff import divergence_np2 as divergence
...
def evaluate_sav(b, B, title, save_path):
    # b : model solution
    # B : reference magnetic field
...
def C_vec(b, B):
    return dot_product(B, b).sum() / torch.sqrt((vector_norm(B)**2).sum() * (vector_norm(b)**2).sum())
...
def C_cs(b, B):
    ...
    M = torch.sum(de!=0.)
    return (1 / M) * torch.divide(nu[de!=0.], de[de!=0.]).sum()
...
def Em_prime(b, B):
    ...
    de = vector_norm(B)
    M = torch.sum(de!=0.)
    return 1 - ((1 / M) * torch.sum(torch.divide(nu[de!=0.], de[de!=0.])))
